...
if __name__ == '__main__':
    ...
    def charge_images(batch_size):
        train_set = torchvision.datasets.CIFAR10(root='./data', train=True, transform=transforms.ToTensor(),
                                                 download=True)
    ...
    def trainConvNet(num_epochs=28, batch_size=32, leaning_rate=0.002, momentum=0.9, dropout=True, p=0.4):
        ...
        print(
            f'Best validation value was: {best_val_accuracy}%, at epoch: {validation_accuracy.index(best_val_accuracy) + 1}')
        ...
        ax.plot(np.linspace(1, num_epochs - 1, num_epochs - 1).astype(int), validation_loss_vals, 'r',
        ...
        ax = fig.add_subplot(111)
        plt.grid(color='0.95')
        ax.plot(np.linspace(1, num_epochs - 1, num_epochs - 1).astype(int), training_accuracy, 'b',
                label='Training accuracy')
        ax.plot(np.linspace(1, num_epochs - 1, num_epochs - 1).astype(int), validation_accuracy, 'r',
